...
- **Domain Layer** (`packages/cli/src/domain/types.ts`): Core types — Node, Edge, NodeKind, EdgeType, Draft, Revision, Proposal, CLIResult.

- **Storage Layer** (`packages/cli/src/fs-model/`): File-based YAML persistence with YAML serialization, file I/O, and path conventions. Each project stores nodes, edges, schemas, revisions, drafts, and proposals as individual YAML files under `projects/<slug>/`.

- **Workspace Layer** (`packages/cli/src/workspace/workspace.ts`): Project lifecycle management, CRUD operations, sequential ID generation via `mp.yaml` counters.

- **Graph Layer** (`packages/cli/src/graph/graph-builder.ts`): In-memory adjacency index with neighbor lookup, forward/backward walking, path tracing, and Mermaid export.

- **Validation Layer** (`packages/cli/src/validation/`): Lint rules (LINT-001/002/003, LINT-CMD/EVT/VIEW/PROC/TRIGGER-001, LINT-EVT-002) and validation rules (EMV-001/010/020/030/031/040/050).

- **Layout Layer** (`packages/cli/src/layout/`): Independent layout program implementing the spec's 4-phase stage-based x-axis model with 3 swim lanes (shared, commandViewModel, event). Modules: `types.ts` (type system + config), `semantic-lift.ts` (canonical edge → display edge mapping), `occurrence.ts` (canonical node → render occurrence + merge), `stage.ts` (4k/4k+1/4k+2/4k+3 stage assignment from anchor), `row-solver.ts` (lane-local row assignment with barycenter sweep), `edge-router.ts` (orthogonal polyline routing), `layout-engine.ts` (initLayout / appendExploreResult / prependExploreResult API). Exported as both a library (`packages/cli/src/layout/index.ts`) and a CLI command (`em layout --focus <node> --direction <both|forward|backward>`).

- **CLI Layer** (`packages/cli/src/cli/`): Command router parsing `em <group> [subgroup] <action> [--flags]` dispatching to 30+ command handlers.
...
- **Dependencies**: `yaml` (parsing and serialization), Express/CORS (local API server)
