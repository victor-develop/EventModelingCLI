...
```plain text
packages/cli/src/workspace/workspace.ts
packages/cli/src/graph/graph-builder.ts
packages/cli/src/layout/layout-engine.ts
packages/cli/src/cli/commands.ts
packages/cli/src/cli/serve.ts
```


Current `packages/cli/src/cli/serve.ts` already exposes:
...
Current `packages/cli/src/cli/commands.ts` already has `layout(ws, focusNodeId, direction, maxHops)`, but that command currently returns a CLI-shaped result:
...
packages/cli/src/terminal-viewer/renderLayoutTable.ts
  owns VisualizationSnapshot -> deterministic table string

packages/cli/src/terminal-viewer/renderLayoutAscii.ts
  owns VisualizationSnapshot -> deterministic ASCII string

packages/cli/src/cli/commands.ts layout()
  becomes a thin command wrapper

packages/cli/src/cli/serve.ts /api/layout
...
Do **not** keep the current layout logic as the canonical implementation inside `packages/cli/src/cli/commands.ts`.


Do **not** copy/paste that logic into `packages/cli/src/cli/serve.ts`.


Do **not** delete `LayoutEngine` or rewrite `packages/cli/src/layout/layout-engine.ts` as part of this refactor.
...
6. Mark `/api/init` as legacy once the viewer app fully migrates to `/api/layout`.
...
- `packages/cli/src/layout/**` remains renderer-agnostic.
...
`packages/cli/src/cli/serve.ts` should therefore be refactored into:
